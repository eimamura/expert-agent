...
- The current implemented baseline includes Phase 3 HTTP API and run persistence.
...
- Preserve redaction before run metadata storage (`question`, `final_answer` in `runs.db`).
...
- Treat `docs/phase_plans/phase3_implementation_plan.md` as the completed Phase 3 summary.
...
- CLI and HTTP API are both supported entry points.
- Single agent only.
...
- `RunStore.update_run` must only accept fields in `_UPDATABLE_FIELDS`.
- HTTP API must return 503 when not initialized, 404 for missing resources.
